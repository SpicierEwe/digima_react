  ...
  return (
    <div className="flex flex-col  md:flex-row p-5 gap-5 my-14 md:my-16 max-w-screen-xl mx-auto">
      {dataArray.map((item, index) => (
        <div
          key={index}
          className={` flex flex-col gap-3 md:gap-5 md:flex-row items-center rounded-lg overflow-hidden shadow-lg mx-5 px-3`}
        >
          {/* image */}
          <div className="h-full w-full  overflow-hidden py-3 md:py-3">
            <img
              src={item.image}
              alt=""
              className="h-1/2 w-full aspect-square rounded-xl shrink-0 object-cover "
            />
          </div>
