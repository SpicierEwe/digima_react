...
function Card({ dataArray }) {
  return (
    <div className="flex flex-col  md:flex-row p-5 gap-0 my-14 md:my-16">
      {dataArray.map((item, index) => (
        <div
          key={index}
          className={` flex flex-col gap-5 md:flex-row items-center rounded-lg overflow-hidden shadow-lg mx-5 px-3 border`}
        >
          {/* image */}
          <img
            src={item.image}
            alt=""
            className="mt-3 md:mt-0 h-40 w-full md:w-fit aspect-square shrink-0 object-cover rounded-xl"
          />
