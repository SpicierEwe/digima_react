...
export default function ImagesSection() {
  ...
  return (
    <div className="flex gap-5 p-9 bg-gray-100 overflow-x-auto overflow-y-hidden">
      <div
        className="relative flex flex-col w-full h-full bg-gray-700 text-white p-8 rounded-lg"
        style={{
      ...
      >
        ...
        <div className="mt-16">
          <h2 className="text-3xl font-bold">{images[0].title}</h2>
          <p className="mt-4">{images[0].subtitle}</p>
        </div>
      </div>
      <div className="flex-1">
        <div className="w-max grid grid-rows-2 grid-flow-col gap-3">
          {images.slice(1).map((image, index) => (
            <div
              key={index}
              className="relative flex flex-col w-96 h-full bg-gray-300 rounded-lg"
            >
              <img
                src={image.src}
                alt="a_photo"
