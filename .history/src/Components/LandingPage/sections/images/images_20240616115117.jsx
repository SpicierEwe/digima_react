...
export default function ImagesSection() {
  ...
  return (
    <div className="flex gap-5 p-9 bg-gray-100">
      <div
        className="relative flex flex-col w-2/4 h-80 bg-gray-700 text-white p-8 rounded-lg"
        style={{
      ...
      <div className="flex-1 overflow-x-scroll">
        <div className="w-max grid grid-rows-2 grid-flow-col gap-3">
          {images.slice(1).map((image, index) => (
            <div
              key={index}
              className="relative flex flex-col w-40 h-40 bg-gray-300 rounded-lg"
            >
              <img
                src={image.src}
                alt="a_photo"
                className="object-cover w-[100rem] h-full"
              />
            </div>
          ))}
        </div>
